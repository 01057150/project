...
async def search_kkbox_api(session, query, retries=1, delay=2):
            ...
            async with session.get(url, headers=headers) as response:
                if response.status == 200:
                    if 'application/json' in response.headers.get('Content-Type', ''):
                        ...
                        logging.info(f"Message: {await response.text()}")
                        return {}
                else:
                    logging.error(f"Unexpected status code: {response.status} for query: {query}")
                    logging.info(f"Message: {await response.text()}")
...
async def process_row(session, index, row):
    ...
    try:
        result = await search_kkbox_api(session, query)
        if result == 'rate_limit_exceeded':
            return 'rate_limit_exceeded'

        # Print the JSON response to see the nested structure
...
async def process_file(file_index):
    ...
    #df_to_process = df_to_process.iloc[500:]  # Skip the first 1000 rows
    
    async with aiohttp.ClientSession() as session:
        tasks = []
        for index, row in df_to_process.iterrows():
            tasks.append(process_row(session, index, row))
        ...
        for result in results:
            if result == 'rate_limit_exceeded':
                ...
                await asyncio.sleep(API_COOLDOWN_PERIOD)
